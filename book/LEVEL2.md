...
第一章では`HTML`の基本といくつかの要素を学びました。
第二章では`HTML`の残りの要素を学び、`HTML`の学習を終了します。
第二章での`HTML`の見どころは {bdg-dark-line}`テキスト：P.59〜63` の「大規模なボックスとしてまとめるタグについて」です。
...
なお、{bdg-dark-line}`テキスト：P.59` では`div`要素のクラス名として`wrapper`（ラッパー、ラッピングするものの意味）を用いています。**ここで`wrapper`はただの名前です。`wrapper`という何か特別な仕組みがあるのではありません。したがって名前が`wrapper`でなくともかまいません**。

```{tip}
なお、`wrapper`は業界の慣習としてよく使われる単語の一つです。
`wrapper`同様によく使用される名前に`container`があります。`container`は「入れ物」という意味です。
```
...
`HTML5`で`section`要素がサポートされて以来、見出しだけで文書の階層を示すのではなく、`section`を配置し、その中に見出しを置く書き方が推奨されています。
...
````{hint}
**高度な話 ― `body`も`section`の仲間です**

なお上のソースコードでは`h1`だけが`section`で囲われていません。
実は`body`も`section`の仲間（`セクショニング・ルート`、セクションの根っ子）と位置づけられているため、見出しを所有することができます。

以下で説明するように、`h1`を`section`で囲ってしまうと意図せざる意味になりますので注意して下さい。

```html
<!-- このように書けば、bodyの見出しが「はじめてのHTML」になります。 -->
<body>
  <h1>はじめてのHTML</h1>
</body>

<!-- 一方、sectionを追加してしまうとbodyの見出しが無いことになってしまいます。 -->
<body>
  <section>
    <h1>はじめてのHTML</h1>
  </section>
...
上記ソースコードでは、次に示すように`h1`を`header`の中に配置していません。その理由を説明します。
```html
<body>
  <header>
    <h1>はじめてのHTML</h1>
...
`header`要素は、セクションの導入部に「見出し**以外のもの**も入れたい」というニーズを満たすためのものです。
% MDNの記事ではこのように説明はされていないが、見出しh1は昔からあり、HTML5でheaderが追加されたことから推測できる

セクションの導入部に「見出しだけ」の場合には、わざわざ`header`要素をマークアップする必要はありません。
...
`header`要素は「[](header-footer)」で改めて説明します

次の例では「見出し`h1`」以外に「主要なナビゲーション`nav`」を導入部に含めたいときに、`header`要素をどうマークアップするかを示しています。
...
このように、`header`要素はどこからどこまでが導入部かはっきりしないときに、その線引き（上記の例では、`h1`から`nav`まで）として利用します。

`header`の使用は全ての`section`に使用可能です。
以下のように、`body`（`body`はセクションの仲間です）だけでなく、全ての`section`（`section`の仲間である`nav`、`article`、`aside`を含む）に`header`要素を利用可能です。
...
```{hint}
「[高度な話 ― headerをいつ入れるか？](when-header)」もお読み下さい。
```
...
- `article` ... **自己完結した部分**を表します。単一の記事や単一のツイート、天気予報のカードなど、この部分だけを切り離しても意味を失わないようなコンテンツを指します。また自己完結とは、これ自身が他の文章の一部ではないこと、つまりこれ自身が頭からお尻までを含む一揃いのコンテンツであるということを説明しています。
	```{hint}
	**`article`はモノという意味です**
	
	`article`を「記事（をマークアップする要素）」と訳して紹介されることがありますが、その理解では不完全です。
	
	確かに「記事」も`article`要素でマークアップするのにふさわしい自己完結したコンテンツの一つですが、その理解では「一つのツイートも`article`である」という説明がつきません。（ツイートは「記事」というには断片すぎます）
	
	つまり記事というのは`article`が指す様々なものの一つに過ぎないのです。
	
	英単語の[`article`の中には「物品、品物、品目、(同種のものの)一個」](https://ejje.weblio.jp/content/article)のような「モノ」という意味があり、このモノという意味で捉える事が重要な鍵になります。
	
	例えば一つひとつのツイートは、その人がその時間に発した言葉というモノとして捉えることができ、`article`要素で括ることが可能になるのです。
	```
...
```{tip}
**フォームでは入力欄を作るだけ...マークアップエンジニアの仕事はそこまでです**
...
業務では、私達{term}`マークアップエンジニア`は`HTML`で入力画面を作るまでが仕事の範囲で、それにアプリケーションを組み合わせて実際に動作するようにするのは、プログラマーの役割です。（{term}`コーディング`済みの`HTML`をプログラマーに渡し、あとはプログラマーに任せます）

したがって、{term}`マークアップエンジニア`は`HTML`の入力画面が作れるようになるところまででOKです。
...
```{hint}
(カ)で変更したスタイルはその場限りのものであり、ページを再読込すればソースコード（元ファイル）に書いてある内容に戻ります。それゆえ、調査のためにどんなにいじったとしてもソースコードを壊すことはなく、どこかを壊す心配をすることなく調査を進められるために非常に便利です。
```
...
% % ```
%
% （高度な話題）この昇段試験では article 要素を使用せよという指示がありますが、本例のようにフッター、ヘッダー、ナビゲーション、およびサイドバーを除いたものがarticle である場合、それ自身が単一の組成物であることは自明なので、わざわざarticle にしなくてもよい（しても構わないが、冗長である）という仕様があります。
