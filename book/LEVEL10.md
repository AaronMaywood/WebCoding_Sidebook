...
第十章では総合試験として３つのデザインの{term}`コーディング`に取り組んでいただきます。

% 総合試験の課題に合格し、ポートフォリオを制作することで **就職支援を受ける段階に到達します。**
総合試験の課題に合格し、ポートフォリオを制作して就職活動に望みましょう。
...
改めて「[](on-portfolio)」を読んで下さい。

ここまでの課題をこなしてきた皆さんはどれほどポートフォリオ作りが進んでいるでしょうか？🤔
...
どのようにポートフォリオを作るかは「[](key-self-promotion-points-for-a-portfolio)」に書いてある事や一般的なウェブ上の情報を活用し、自身の判断で進めて下さい。

```{hint}
作成したポートフォリオがあれば希望する職種、勤務体系のヒアリングとともにキャリア・コンサルタントから就職活動のアドバイスを受けることができます。
```
````
...
% 一方で、`HTML/CSS`の{term}`コーディング`をじっくり見てもらうには、ブラウザーで直接閲覧できるウェブサイト形式が適しています。
% 
...
% - https://www.resume.id/
% このサービスを使用してどんなポートフォリオが作成できるかは、実例リンク集である「[トレンドをチェック](https://www.resume.id/trends)」から確認することができます。
% 
...
(key-self-promotion-points-for-a-portfolio)=
### ポートフォリオに盛り込みたい自己アピール要素

{term}`コーディング`した作例に併せて以下の要素をアピールしましょう。

#### コーディングに関する自身の考えやこだわった点

{term}`コーディング`作業の中では以下のような点であれこれ悩んだり試したりするものです。その経験を通して会得したことや自身の取り組みを示しましょう。
...
- 模範解答を読んで学び取ったこと
  - 他人の{term}`コード`をデベロッパーツールで調査するスキルがあること
- ウェブ検索で調べて学習する力があること
- 学習態度
...
**デベロッパーツールの使いこなし、デバッグの能力は現場でも特に求められるスキルの一つです。デベロッパーツールは機能が多いので、{term}`コーディング`作業を通じて少しづつ習得するようにしましょう。**
...
以下にしめす`A.`、`B.`、`C.`の３つのウェブデザインの`HTML/CSS`{term}`コーディング`を行って下さい。いずれもレスポンシブ対応が必要です。
...
- {term}`コーディング`ができたら動作確認を行って下さい。
  - レスポンシブ対応ができているかどうかの確認もお願いします。ブレークポイント前後の幅（例えばブレークポイントが`345px`の場合には、`345px`と`346px`が前後の幅です）での確認も行って下さい。
	...
	```
	````
  - ブレークポイント以外の幅での表示は{term}`マークアップエンジニア`に任されています。`320px`〜`1980px`に渡る表示に耐えられるように実装してください。
...
2. 完成見本
	- https://adjustacademy.com/webcoder/7d8b5ed936e7752f1011f15390242d27.zip
		...
		**2. 完成見本**の`css/style.css`は読みにくいように**難読化**しています。（難読化はソースの上だけで、ページの解析に必要なデベロッパーツール上からは普通に見えます。）
		...
		あくまでもこの作例からデベロッパーツールを通じて学びとったことをベースに、自身の知識や考えに基づいて{term}`コーディング`を行っていただくための配慮です。
		% ↓勉強したことをそのまま再現する可能性を潰すかもしれないので、この文章はボツ
		% **その結果として完成見本とは異なる{term}`コーディング`になるはずです。（`CSS`は表現力が高いためにこのようなことが起こり得ます）**
		```
...
- 次に **「1. 素材」** の素材（`index.html`や`css/style.css`）に手を加えて **「2. 完成見本」** の形になるまで{term}`コーディング`して下さい。
%   - 実際には **「2. 完成見本」** と **「1. 素材」** とを交互に見比べながら **少しづつ** 作業を進めて下さい。
%     - いきなり全部を仕上げようとするのは無茶です。不具合が混入したときに疑う箇所が多くなりすぎるからです。`CSS`でレイアウトされる要素を一つ一つバラバラにし、一つの要素を{term}`コーディング`したら直ちにブラウザーで確認する、という操作を積み重ねれば間違えた時にそれとすぐ知ることができます。**これは複雑さを減らす方法で、プロの用いる方法です**。
- 一度{term}`コーディング`が完成したら、今度はそれを **「2. 完成見本」** の`CSS`を見ずに（デザインカンプの確認のために参照するのはかまいませんが、`CSS`の仕組みを見ることはしないで下さい。）再度イチから{term}`コーディング`して下さい。`CSS`の不明点に関してはしらべながらでかまいません。 **このレベルに到達することが業務に就くために必要な要件で、妥協せず取り組んでください。**
...
- ファイル名の指示
	...
	- CSSは`css/style.css`として下さい。
- {term}`コーディング`ができたら動作確認を行って下さい。
  - レスポンシブ対応ができているかどうかの確認もお願いします。
  - ブレークポイント前後の幅（例えばブレークポイントが`345px`の場合には、`345px`と`346px`が前後の幅です）での確認も行って下さい。
	ブレークポイントの画面幅を確認するには、デバイスモードを使用して下さい。
	````{dropdown} デバイスモードを使用してビューポートの幅を切り替える方法を確認する
	```{include} cards/devtools/device-mode.md
	```
...
2. レスポンシブ対応されているかどうかを`320px`〜`1980px`の幅を用いて確認します。
	- デザインカンプでは特定のブレークポイントが使用されていますが、それ以外の画面幅に関しては{term}`マークアップエンジニア`に任されています。`320px`〜`1980px`に渡る表示に耐えられるように実装してください。
3. 適切な`HTML`マークアップや、`CSS`{term}`コーディング`が施されているかどうかを確認します。
...
% {{TODO}} ベータ版につき、コメントアウト
% 総合試験の課題提出が終わったら、「[](making-portfolio)」に沿ってポートフォリオ作りを進めて下さい。
総合試験の課題提出が終わったら、「[](making-portfolio)」に沿ってポートフォリオ作りを進めて自身の就職活動に活かす準備をしましょう。
...
`HTML/CSS` {term}`マークアップエンジニア`の道は、ここまでが入り口です。
...
これらの配付素材をもとに{term}`コーディング`し、ポートフォリオに盛り込みましょう。
...
- [動くWebデザインアイディア帳](https://coco-factory.jp/ugokuweb/)
	さまざまな動くデザインを紹介しています。HTML/CSSの{term}`コード`もありますので、その作り方を学んで見て下さい。
	（JavaScript、jQueryといったプログラム技術が必要とされるものも含まれていますが、`HTML/CSS` だけでできるものも多数あります）

## ジェネレーター

画面上で操作した結果をそのまま`HTML`や`CSS`の{term}`コード`として取得できる「ジェネレータ（`HTML/CSS`の生成器）」とよばれるツールがあります。これを利用すると{term}`コーディング`の生産性が向上します。
...
「もっと初心者向けの噛み砕いた情報が欲しい」「新しいCSSプロパティには何があるか？」など、必要に応じて自ら検索し、`HTML/CSS`{term}`マークアップエンジニア`の道を深めて行って下さい。
