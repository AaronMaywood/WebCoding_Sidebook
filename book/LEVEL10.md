...
- コーディングができたら動作確認を行って下さい。
  - レスポンシブ対応ができているかどうかの確認もお願いします。ブレークポイント前後の幅（例えばブレークポイントが`345px`の場合には、`345px`と`346px`が前後の幅です）での確認も行って下さい。
	...
	```
	````
  - ブレークポイント以外の幅での表示はコーダーに任されています。`320px`〜`1980px`に渡る表示に耐えられるように実装してください。
...
2. レスポンシブ対応されているかどうかを`320px`〜`1980px`の幅を用いて確認します。
	- デザインカンプでは特定のブレークポイントが使用されていますが、それ以外の画面幅に関してはコーダーに任されています。`320px`〜`1980px`に渡る表示に耐えられるように実装してください。
