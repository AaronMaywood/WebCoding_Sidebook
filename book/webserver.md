...
```{tip}
**なぜさくらのレンタルサーバーですか？...別にこだわる理由はありません**

さくらのレンタルサーバーを勧めるのはサーバー大手の`SAKURA internet`が提供していて、安価で信頼のおけるサービスだからです。
ただし同じく大手かつ安価なレンタルサーバー会社は他にもあり、実のところどこでもよいです。

ただし、本書に出てくる手順ではさくらのレンタルサーバーの場合についてのみ記載しています。その点だけ注意してください。
``` 

有料サーバーは今すぐに必要ではありませんが、この章で扱う`FTP`の練習を行いたい場合や、ポートフォリオづくりを行う第10章では必要になりますので心の準備だけをしておいて下さい。
...
````{hint}
`FTP`の練習をしたい場合には先にレンタルサーバーを用意しておいてください。
（本書で推奨するさくらのレンタルサーバー・ライトプランを申し込んでおいてください。）

```{hint}
**既ににお持ちの別の会社のサーバーでもかまいません**

ただし説明はさくらのレンタルサーバー・ライトプランを基にします。その他のサービスを利用されている方は、それぞれのサービスの案内に従ってください。
```
````
