...
サーバー大手の`SAKURA internet`が提供しており、安価で信頼のおけるサービスだからです。
ただし、同じく大手かつ安価なレンタルサーバー会社は他にもあり、実のところどこでもよいなと感じています。
強いて言うなら筆者が`SAKURA internet`を利用しがちだという程度です。
```

有料サーバーは今すぐに必要ではありませんが、この章で扱う`FTP`の練習や、ポートフォリオづくりを行う第10章では必要になりますので心の準備だけをしておいて下さい。
