...
```{hint}
**実際に扱うのは透明度ではなくてその逆の不透明度です**

ここで扱う話題は厳密には透明度というよりその逆の**不**透明度です。

透明度は数値が高くなるに連れ徐々に透明のものとして扱う概念を指し、不透明度はその逆で数値が低い時に透明で、数値が高くなるに連れて不透明になる（画面上にくっきりと表示される）という違いがあります。

どちらも「どのくらい透明にするか？」という操作をする上では同じ概念を単に言い換えたものと捉えて差し支えありません。
従って「透明度」と言ってもあながち間違いではありませんし、一般的にはそのほうが理解しやすいでしょう。

しかし`CSS`で扱っている概念は**厳密には**不透明度の方なのでその旨補足しておきます。
```

```{hint}
**不透明度の具体的な数値表現**

不透明度はどのくらい不透明であるか？を扱い、`CSS`では`0`(透明=完全に透明)〜`1.0`(全く透明でない)の範囲で指定します。
`0.5`では半透明になります。
````
...
更にややこしい話ですが、`CSS`では「不透明度」という概念を２つの用語で使用しています。

- 一つが`opacity`（オパシティ）で、ずばり不透明度を指す言葉です。
- もう一つがα値（アルファ値）で、これは文脈によって透明度・不透明度両方の意味で使われる用語ですが、`CSS`では不透明度の意味で使われています。
````

不透明度をコントロールできるものには２種類あります。
1. 要素そのもの ... `opacityプロパティ`で指定する
2. 背景や文字など色指定ができるもの ... `rgba()`で色指定にアルファ値を含めた透明色を指定する
	```{tip}
	CSSで色を扱う`rgba()`については「[](color)」で紹介している`rgb()`に更にアルファ値`a`を加えたものです。
	```
...
{term}`コーディング`ガイドラインとは、`HTML`や`CSS`を書く時の現場のルールをまとめたものです。

`HTML`や`CSS`は高い表現力を持つため、様々な書き方が許容されてしまい、他の人が理解しにくい{term}`コード`を作るのも容易です。

個人で制作している時にはガイドラインは不要ですが、複数人で{term}`コーディング`をしたり、また他の人が作成したページを保守するといった場合には、あらかじめ何らかの取り決めに沿って{term}`コード`の品質を均一化しておく必要が生じます。

そこで現場や会社で{term}`コーディング`上の規約をまとめたガイドラインを作り、{term}`マークアップエンジニア`に配付してそれに沿った{term}`コーディング`を求めるということが行われるようになりました。

{bdg-dark-line}`テキスト：P.169〜173` はそんなガイドラインの一例です。（サンプルなので、内容自体は`HTML5`以前の古いものです。）

「改行{term}`コード`を原則`CR+LF(Windows)`とする」など未説明の不明なルールも多々見られますが、全体を一読して、現場で求められるルールの雰囲気を読み取って下さい。
...
- {bdg-dark}`指示` 作成した`position`ファイル一式を{{OFFICE}}に提出して下さい。
    - 提出は次のようにして下さい。
        1. レンタルサーバーを借りている場合
            ...
            - 以下の文言テンプレートを利用し{{OFFICE}}にメールにて提出して下さい。
				...
				https://xxxxxx/position/2/index.html
				https://xxxxxx/position/3/index.html
                ```
        2. レンタルサーバーを借りていない場合
