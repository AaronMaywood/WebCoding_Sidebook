...
このことを発見した当時のデザイナーや{term}`マークアップエンジニア`達が段組みなどの高度な表現を試みるようになり、ウェブの表現は当初の論文からかけはなれた、いわゆる「ウェブデザイン」の分野が切り開かれました。
...
```{warning}
ただし、業務ではどちらの{term}`コード`も扱う機会があります。
少し前に書かれたウェブサイトの{term}`コード`の保守や、{term}`マークアップエンジニア`の意見によっては今でも`62.5%`が使用されます。

**`62.5%`を使用した{term}`コード`でも、そうでない{term}`コード`でも両方扱えるようになる**のが望ましいです。
