...
- `Web`制作などの制作作業では、拡張子や隠しファイルの表示は必須です。{bdg-dark-line}`テキスト：P.11` の通りに設定しましょう。

```{attntion}
「拡張子」が隠れた状態だと、ファイルを保存した時に、見えない拡張子が付いてしまっていることに気づくことができません。

例えば`index.html`というファイル名で保存した時に、実際は`index.html.html`となってしまった時に気づく事ができません。そして`index.html`を開く時に「ファイルがありません」と言われてしまったりします。
また、`index.html`というファイル名で保存した時に、実際は`index.html.css`というように「CSSファイル」として保存されてしまう
TODO ここから
```
