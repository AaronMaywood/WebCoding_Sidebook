...
```{tip}
インターネットには`Web`以外にも`eメール`や`LINE`、`Zoom`等の様々な異なる仕組みのサービスがあります。
`Web`はそのうちの一つです。
```

`Web`の特徴は文書同士が「リンク（`HyperLink`、ハイパーリンク）」という仕組みで紐付けられていることです。
文書同士がリンクで繋がり合い、`Web`上にあるページが蜘蛛の巣のように複雑な網目状になっていることから`Web`（蜘蛛の巣）と名付けられました。

`Web`の正式名称は`World Wide Web`（ワールドワイドウェブ、世界中に広がった蜘蛛の巣）といい、その略称は`WWW`（ダブリュ・ダブリュ・ダブリュ）です。また発音のしやすさから`WWW`を`Web`と言うことが一般的です。
...
ウェブサイト制作を行う上では細かい話はあまり重要ではなく、作成したウェブサイトをインターネット上にある`WWWサーバー`（`Webサーバー`）と呼ばれるパソコンにアップロードするとそのページが公開され、どこからでもアクセスできるようになる、という点を押さえて置いて下さい。

また、`Webサーバー`にファイルをアップロードする際には、`Webサーバー`のことを「リモート（遠隔地）」と呼びますので、この言葉も押さえておいて下さい。それに対して自分自身のパソコンをローカル（地元）と呼びます。
...
- ファイルのアイコンが拡張子に紐付けられた**既定のアプリケーションのアイコン**を示しています。したがってアイコンを見ればそのファイルの既定のアプリケーションがわかります。
  ```{figure} https://i.gyazo.com/ad4e0e8e9c85fea6eb08e8bf45d3fc85.png
...
入れ子
  ...
  % （あまり使用しない用語ですが、`meta`から見た時の`html`を親の親で祖先、その逆で`html`から見た`meta`を子の子で子孫ということがあります。）
...
- `dl`は`description-list`（説明リスト）という意味です。
  - テキストにある「定義リスト」は過去の名称で、現在は「説明リスト」に改められています。
    - 過去は用語集を作成するためのものでしたが、現在では用語以外のペアとなる一般的な情報（例:血液型）を記述可能となったため、「説明リスト」に改められました。
...
`div`も`span`も`CSS`**でスタイルを付ける範囲を示すために用いる要素**で、Webデザインでは特に`div`が多用されます。
...
```
クラス属性とは、クラス名と呼ばれる名前を要素に付与する機能のことです。クラス名には製作者が考えた名前を自由に指定できます。

このように`span`（`div`）を異なるスタイルにするには、**`クラス属性`で異なる名前を付けることで行います。`span`や`div`は名前を付けて使うのが通常の使い方になります。**
````
...
`alt`とは`altenative`（オルタナティブ、代替物）の略です。画像の現物が無い、または使用できない場合の画像の代わりとして使用する文章を指定します。
...
- 続いて`aaa/images/index2.html`を開き、３つの`img`要素があることを確認し、`Go Live`で３つの画像が表示されるかをプレビューします。
  ...
  % ショートカットは{bdg-dark}`Alt+O` のち{bdg-dark}`Alt+L` 
  ```
  ````
  - ブラウザ上に３つの画像があればOKです。
...
ここまでで完成した場合にどう画像が見える化を理解したと思います。
...
- `gazo1.jpg`というファイルに対し、`bbb/index0.html`から見た場合には一度一つ上のフォルダ（`../`）に行った後、そこから`aaa`フォルダの中に入り（`aaa/`）ます。一つ上に行き、`aaa`フォルダの中に入るということを`../aaa/`と記します。したがって`gazo1.jpg`を指すには`.../aaa/gazo1.jpg`とします。
- `gazo2.jpg`、`gazo0.jpg`も同様の考えでファイルの場所を
- この応用として、{bdg-dark-line}`テキスト：P.45` (H)では、`aaa/images/index2.html`から`gazo0.jpg`を見ています。`index2.html`から２つ上のフォルダを示すには、一つ上を指す`../`を二回繰り返して`../../`と表記します。２つ上にある`bbb`フォルダの中を表すには`../../bbb/`となります。

相対パスの表現をまとめると次の３つとなります。
...
これに対し、どのファイルからでも同一の表現でファイルの有りかを示すことのできる絶対パス（絶対URL）というものがあります。絶対パスは[](absolute-url)で学びます。
```
````
...
- 今度は{bdg-primary-line}`教材：HTML教材/04_画像のパス表記例/練習2` フォルダを`VS Code`フォルダで開いて作業を始めて下さい。
  - `練習1`〜`練習4`フォルダはいずれも内容は同じです。`練習1`フォルダで練習を終えたら、今度は`練習2`で最初からおさらいする、のように使用した下さい。

% {{TODO}} ファイルパス、URLをもっと詳しくする？↑上の練習で相対パスは十分だと思う
...
ファイルパスが使用できる`a`要素や`img`要素には`http:`または、`https:`から始まる`URL`を指定することができます。

```{glossary}
URL
  インターネット上にあるリソース（ファイル）を指し示す住所のことです。主に`http:`および`https:`から始まります。
```
...
あるファイルの場所を基準にした**相対パス**と異なり、`http:`や`https:`から始まる`URL`はどこからでも決められた一意の場所を示す絶対的な表現であるため、「**絶対パス**」（または**絶対URL**）と言います。
...
`URL`を指定できるのは、{bdg-dark-line}`テキスト：P.47` で説明している`a`要素に限りません。
ファイルパスを指定できるものなら何でも使用できます。
例えば`img`要素です。`img`要素に`URL`を指定し、インターネット上にある画像を埋め込むことも可能です。

たとえば、Wikipedia上の画像である"https://upload.wikimedia.org/wikipedia/commons/4/4e/Sir_Tim_Berners-Lee_%28cropped%29.jpg"の`URL`を使って`img`要素を記載できます。
...
{bdg-primary-line}`教材：HTML教材/08_ページ内リンク/練習１/` の教材を`VS Code`で開いて{bdg-dark-line}`テキスト：P.49〜50`の通りに作業をしてみてください。
