...
リキッドレイアウトの別名にフリュード（`fluid`、流体）レイアウトがあり、海外ではこちらの呼び方が多いようです。このフリュードも液体になぞらえた用語が選ばれていますね。

なお、 **CSSは文字やブロックなどの表示物をレイアウトという枠に流し込むという世界観** を持っていて、「流れ」（水の流れ）を感じさせる比喩表現が多く有ります。通常のフロー（`flow`は「流れ」）や`float`（浮き）、そしてリキッドやフリュードなどの語がそうです。
````
