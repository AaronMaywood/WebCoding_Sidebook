...
この方法にはデバイスに応じた最適なページが作りやすいというメリットはあるものの、サイトが分かれてしまうことでSEO効果が弱まるというデメリットを抱えていました。
...
このレスポンシブウェブデザインの手法はGoogleが推奨したことで業界に普及したという歴史もあります。

```{tip}
**レスポンシブウェブデザイン(Responsive Web Desibn, RWD)**

レスポンスとは**敏感に反応して応じる**という意味で、レスポンシブウェブデザインは「ページ自体が閲覧に利用している各種のデバイスに反応して最適な表示に切り替えるウェブデザイン手法」と言った意味です。
...
なお海外ではリキッドレイアウトのことをフリュード（`fluid`、流体）レイアウトとも呼ばれます。フリュードも液体になぞらえた用語が選ばれていますね。
...
````{hint}
追加するコードの意味はテキストにコメント書きしてあることくらいの大雑把な理解で`OK`です。
全てを理解して使用することが理想ではありますが、現実問題として、以下の理由からそうすることが普通です。
