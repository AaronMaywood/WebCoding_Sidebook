...
{bdg-dark-line}`テキスト：P.232〜` から始まる内容は様々な話題が込み入っているので、テキストに先立って集中的に要点解説をします。
その後の「[](smartphone-site-text)」からテキストに沿って補足をしますので、それまで要点解説をお読み下さい。

(pointcheck-multidevice)=
## 要点解説

テキストではウェブサイトをスマートフォンやタブレットなどの様々なデバイスにも対応する「マルチデバイス対応」として次の話題を学びます。
...
本テキストは「マルチデバイス対応」のテキストですが、具体的なマルチデバイス対応の手法として**レスポンシブウェブデザインを学ぶテキスト**となっています。
...
- `(min-width: 600px)` ... `max-width`と反対の意味で「幅が`600px`以上の時」です。
	```{hint}
...
`iPhone SE`ではデバイスの画面幅は`375px`なのに対し、ビューポートの幅は`980px`（一般的なスマートフォンはだいたい`980px`です）となっています。

つまり、スマートフォンでは画面の大きさに２種類あるのです。実機（デバイス）の画面と、内部的なビューポートの画面です。
そして、ブラウザーはビューポートの画面`980px`に対しPC版のページを描画し、それをデバイス画面の`375px`まで縮小して表示します。

前述のように、このPC版サイトを縮小表示する仕組みはレスポンシブ対応済みのページには却って邪魔です。
...
これによりビューポートの幅が実機の幅である`375px`になるため、メディアクエリーの条件も小さい画面サイズが有効になります。

末尾の`initial-scale=1.0`の意味するところは、スマートフォンの画面を横に倒した時にコンテンツが勝手に拡大表示されないようにするための設定で、
拡大縮小率を（横に倒したときも）１倍に固定するものです。
固定と言っても指で拡大縮小することまで禁止するものではありませんので、利用者が必要に応じて拡大することは可能です。 
...
リキッドレイアウトは画面幅に応じるレスポンシブのもう一つのテクニックで、
メディアクエリーがブレークポイントによって段階的にCSSを切り替えるのに対し、
リキッドでは「画面幅の80%」などというパーセンテージ（表示比率）を指定する手法です。
...
ここで`CSS`の単位`%`の意味をおさらいしておきますが、単位`%`は、常に**親要素に対しての割合という意味**です。
...
同じものを海外では引き出し（drawer）という意味でドロワー（ドロアー、ドローアなど）と呼ぶようです。
```
...
ビューポートの設定も忘れないでくださいね。

追加の話題として、必要に応じて導入するハンバーガーメニューの仕掛けを紹介しました。
...
追加するコードの意味はテキストにコメント書きしてあることくらいの大雑把な理解でOKです。
