...
このテキストでは様々なテクニカルな話題が込み入っているので、読みやすくするためにテキストに先立って集中的に要点解説をします。
...
ウェブサイトをスマートフォンでも閲覧できるようにするには、大きく２通りの方法があります。

１つ目はかつて行われた手法で、PC版のウェブサイトとは別の版としてスマートフォン用のサイトを構築する方法です。
% （スマートフォンはこちら、PC版はこちら、などのリンクが提供されました）
...
それはページの中に、PCやスマートフォン等の各種デバイスに応じてCSSを切り替えるなどのカラクリを内蔵させる方法です。
...
レスポンスとは**敏感に反応して応じる**という意味で、レスポンシブウェブデザインは「ページ自体が各種のデバイスに応答して最適な表示を行うウェブデザイン手法」と言った意味です。
**長いので単に「レスポンシブ」と呼ばれます。**また、ウェブサイトをレスポンシブ化することを広く**レスポンシブ対応**と呼ばれています。
```

本テキストは「マルチデバイス対応」のテキストですが、具体的にはレスポンシブウェブデザインの手法を学ぶテキストとなっています。
...
**「スマートフォンサイト制作実習」P.2-3**では、[](pointcheck-multidevice)で説明したことがまとまっています。
おさらいしながら読んで下さい。
