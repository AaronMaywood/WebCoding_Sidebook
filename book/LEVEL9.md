...
```html
<!-- この一行をHTMLに追加するだけでレスポンシブが有効になります -->
<meta name="viewport" content="width=device-width,initial-scale=1.0">
```
...
なお、**CSSは文字やブロックなどの表示物をレイアウトという枠にに流し込むという世界観**を持っていて、「流れ」を感じさせる比喩表現が多く有ります。通常のフロー（flowは「流れ」）やfloat（浮き）、そしてリキッドやフリュードなどの語がそうです。
````
