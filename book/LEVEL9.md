...
同様に、下限を決める `min-width` プロパティというものもあります。
````
...
{bdg-dark-line}`テキスト：P.244〜246` で説明している{term}`コード`をこの通りに記述して動作するところまでもっていきましょう。
指示通り設置し、動作確認ができればOKです。

````{hint}
追加する{term}`コード`の意味は {bdg-dark-line}`テキスト：P.244` の赤字コメントに記されている内容程度の理解で充分です。
深追いしなくてかまいません。

**理由**
- 外部から持ってきた何らかの仕組みは、その仕組みの作者の説明に従って「設置するだけ」が普通の使い方になります。（リセット`CSS`もそうでした。概要の理解は必要ですが、詳細の理解までは通常必要ありません。）
- 以下の`script`要素の部分は、`HTML`でも`CSS`でもない、`JavaScript`（ジャバスクリプト）というプログラム言語の記述で、みなさんのような`HTML`と`CSS`の{term}`マークアップエンジニア`には専門外だからです。
```html
...
なお、{bdg-dark-line}`テキスト：P.267〜286`（教材としては{bdg-primary-line}`SCSS教材/完成版`）にある完成版の{term}`コーディング`は、`LEVEL8`昇段試験の模範解答とは以下の点で若干異なります。
- リキッドレイアウトに対応している
...
- デザインカンプでは`1441px`、`375px`の２つの画面幅についての指示がありますが、それ以外の画面幅に関しては{term}`マークアップエンジニア`に一任されています。`320px`〜`1980px`に渡る表示に耐えられるように実装してください。
	```{tip}
...
{term}`コーディング`できたら、以下の画面幅の条件で表示崩れがないかの検証をお願いします。
- 1440px周辺として
  - `1441px`以上
  - `1440px`の時
  ```{hint}
...
- デザインカンプ
	...
	- また、`GreenCamp-main/README.md` に{term}`コーディング`に必要な情報があります。
...
- {bdg-dark}`指示` 作成した`responsive`、及び`GreenCamp`ファイル一式を{{OFFICE}}に提出して下さい。
    - 提出は次のようにして下さい。
        1. レンタルサーバーを借りている場合
            ...
            - 以下の文言テンプレートを利用し{{OFFICE}}にメールにて提出して下さい。
                ...
                ○ ○ です。
				Portfolio編
				https://xxxxxx/responsive/index.html
				GreenCamp編
				https://xxxxxx/GreenCamp/index.html
                ```
        2. レンタルサーバーを借りていない場合
