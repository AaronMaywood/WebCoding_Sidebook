...
```{tip}
{bdg-dark-line}`テキスト：P.267〜286` では、第八章の昇段試験で扱った「SAMPLE SITE」が出てきており、それを`SCSS`で書いたらどうなるかを説明しています。
第八章の昇段試験での経験が、`SCSS`を学ぶ時の助けになると思います。なんと言っても第八章の昇段試験に必要な`CSS`の知識がバッチリあるわけですからね。

なお、{bdg-dark-line}`テキスト：P.267〜286`（教材としては{bdg-primary-line}`SCSS教材/完成版`）にある完成版の{term}`コーディング`は、第八章昇段試験の模範解答とは以下の点で若干異なります。
- リキッドレイアウトに対応している
