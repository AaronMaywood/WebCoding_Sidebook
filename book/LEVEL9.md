...
{bdg-dark-line}`テキスト：P.232〜` から始まる内容はテクニカルな話題が込み入っているので、テキストに先立って集中的に要点解説をします。
その後の「[](smartphone-site-text)」からテキストに沿って補足をしますので、それまで要点解説に集中して下さい。
...
`display:none`が指定されたボックスは、あたかも`HTML`中に書いてなかったかのように取り除かれます。レスポンシブでは、ある画面幅の時だけは見せて、それ以外は隠すという用途でよく用いられるテクニックです。
