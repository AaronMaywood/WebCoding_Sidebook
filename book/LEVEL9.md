...
```{tip}
{bdg-dark-line}`テキスト：P.267〜286` では、`LEVEL8`の昇段試験で扱った「SAMPLE SITE」が出てきており、それを`SCSS`で書いたらどうなるかを説明しています。`LEVEL8`の正田試験での経験が、`SCSS`を学ぶ時の助けになると思います。なんと言っても`LEVEL8`の昇段試験に必要な`CSS`の知識がバッチリあるわけですからね。
